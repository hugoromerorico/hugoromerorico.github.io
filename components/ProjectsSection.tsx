    ...
    <div className="flex-1 p-4">
      <Card>
        <CardContent className="p-6">
          <img src={projectsData[currentProjectIndex].image} alt={projectsData[currentProjectIndex].title} className="w-full h-48 object-cover mb-4 rounded-md" />
      ...
      <div className="flex justify-between mt-4">
        <Button variant="outline" onClick={() => setCurrentProjectIndex((prev) => (prev - 1 + projectsData.length) % projectsData.length)}>
          <ChevronLeft className="h-4 w-4 mr-2" /> Previous
        </Button>
        <Button variant="outline" onClick={() => setCurrentProjectIndex((prev) => (prev + 1) % projectsData.length)}>
